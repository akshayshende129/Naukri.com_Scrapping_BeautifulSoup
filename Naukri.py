...
if __name__ == '__main__":				
	...
	#Displaying Bar Plot
	def showBarPlot(colname,xlabel,ylabel,title,saveName='temp'):
		naukriDf = naukri.copy()
		...
		p.set(xlim=[0,max(df.Count)+min(df.Count)], xlabel=xlabel, ylabel=ylabel,
			   title=title)
	...
	#Jobs by companies
	showBarPlot('CompanyName',title='Jobs In Various Companies',xlabel='Number of Jobs',ylabel='Companies',saveName='companies')
	#Jobs by location
	showBarPlot('Location',title='Jobs In Various Locations',xlabel='Number of Jobs',ylabel='Location',saveName='location')
